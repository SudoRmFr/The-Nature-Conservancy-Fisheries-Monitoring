...
parser.add_argument('--fold', type=int, required=True, help="0 for 0.1-valid-size holdout; 1~4 for 4-folds.")
...
def main():
    ...
    train_dataset, validate_dataset = get_trainval_datasets(config.tag, config.image_size, args.fold)
...
def train(**kwargs):
    ...
    for i, (X, y) in enumerate(data_loader):
        ...
        batch_info = 'Loss {:.4f}, Raw Acc ({:.2f}, {:.2f}), Aug Acc ({:.2f}, {:.2f}), Aux Acc ({:.2f}, {:.2f}), lr {:.5f}'.format(
            epoch_loss,
            epoch_raw_acc[0], epoch_raw_acc[1],
            epoch_crop_acc[0], epoch_crop_acc[1],
            epoch_drop_acc[0], epoch_drop_acc[1],
            now_lr)
...
def validate(**kwargs):
    ...
    with torch.no_grad():
        for i, (X, y) in enumerate(data_loader):
            ...
            for ii in range(len(y)):
                one_y = y[ii]
                one_pred = softmax_y_pred[ii]
                one_pred_prob = one_pred[one_y]
                all_sum += torch.log(one_pred_prob).item()
            all_sum /= len(y)
            all_sum *= -1
            log_loss_info = f'Valid log-loss evaluation: {all_sum}'
            print(log_loss_info)
            logging.info(log_loss_info)
    ...
    if args.local_rank == 0:
        ...
        if epoch_acc[0] > best_acc:
            ...
            save_model(net, logs, 'model_best_acc.pth')

        if aux_acc[0] > best_acc:
            best_acc = aux_acc[0]
            save_model(net, logs, 'model_best_aux_acc.pth')
...
def save_model(net, logs, ckpt_name):
    torch.save({'logs': logs, 'state_dict': net.module.state_dict()}, os.path.join(config.save_dir, ckpt_name))
