...
def main():
    # Load dataset
    ##################################
    train_dataset, validate_dataset = get_trainval_datasets(config.tag, config.image_size, fold=args.fold)
...
def visualize(**kwargs):
    ...
    with torch.no_grad():
        for i, (X, y) in enumerate(data_loader):
            ...
            _, attention_maps = net.visualize(X)
...
def test(**kwargs):
    ...
    with torch.no_grad():
        for i, (X, y) in enumerate(data_loader):
            ...
            # if i == 0:
            #     print(sum_preds[0])
            # preds = preds * 0.99 + 0.01
            # if i == 0:
            #     print(preds[0])
            # new_sum_preds = torch.sum(preds, dim=1)
            # if i == 0:
            #     print(new_sum_preds[0])
            # preds /= new_sum_preds.view(len(new_sum_preds), 1)
            # if i == 0:
            #     print(preds[0])
            # pred = preds
            ...
            for ii in range(len(pred)):
                ...
                # print(f'1.{pred_with_idx[0][1]}\n2.{pred_with_idx[1][1]}\n3.{pred_with_idx[2][1]}')
                # Make new one_pred
                #     No.1~3 => 0.25
                #     No.4~8 => 0.05
                new_one_pred = [0., 0., 0., 0., 0., 0., 0., 0.]
                new_one_pred[pred_with_idx[0][1]] = 0.25
                new_one_pred[pred_with_idx[1][1]] = 0.25
                new_one_pred[pred_with_idx[2][1]] = 0.25
                new_one_pred[pred_with_idx[3][1]] = 0.05
                new_one_pred[pred_with_idx[4][1]] = 0.05
                new_one_pred[pred_with_idx[5][1]] = 0.05
                new_one_pred[pred_with_idx[6][1]] = 0.05
                new_one_pred[pred_with_idx[7][1]] = 0.05
                new_pred.append(new_one_pred)
            ...
            for ii in range(len(y)):
                one_y = y[ii]
                one_pred = pred[ii]
                one_pred_prob = one_pred[one_y]
                all_sum += torch.log(one_pred_prob).item()
            all_sum /= len(y)
            all_sum *= -1
            print(f'Log loss evaluation: {all_sum}')
